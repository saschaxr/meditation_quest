...
/*!*****************************************************!*\
  !*** ./src/domain/worklets/AudioOutputProcessor.ts ***!
  \*****************************************************/
...
class AudioOutputProcessor extends AudioWorkletProcessor {
    ...
    MAX_AUDIO_BUFFER_LENGTH = 360; // The maximum number of audio blocks to buffer
    MIN_AUDIO_BUFFER_LENGTH = 180; // The minimum number of audio blocks to have before starting to play them.
    ...
    // @ts-ignore
    process(inputList, outputList /* , parameters: Record<string, Float32Array> */) {
        const FLOAT_TO_INT = 32767;
        // Grab the next block of audio to play.
        let audioBlock = undefined;
        if (this._isPlaying) {
        ...
        for (let channel = 0; channel < channelCount; channel++) {
            ...
            for (let i = 0; i < sampleCount; i++) {
                let sample = 0;
                if (audioBlock) {
                    sample = audioBlock[i * 2 + channel] / FLOAT_TO_INT;
                }
                samples[i] = sample;
            }
        }
